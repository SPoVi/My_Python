...
def segment_supervised(image_descriptor,model,normalize_data,scaler):
    from sklearn.preprocessing import StandardScaler
    X = numpy.reshape(image_descriptor, (image_descriptor.shape[0] * image_descriptor.shape[1], image_descriptor.shape[2]))
    if normalize_data:
        X= scaler.transform(X)
    probability_image = model.predict_proba(X)
    segmentation = model.predict(X)

    return  numpy.reshape(probability_image,(image_descriptor.shape[0],image_descriptor.shape[1],probability_image.shape[1])),numpy.reshape(segmentation,(image_descriptor.shape[0],image_descriptor.shape[1]))
...
if __name__ == '__main__':
    ...
    if show_loading:
        print('Esta imagen muestra los descriptores LBP (Local binary patterns) que indican textura')
        # show image
    ...
    image_color_descriptor = extract_image_descriptor_mean_var(rgb2lab(imageRGB.copy()), window_size, step)
    ...
    image_descriptor_real_size = skimage.transform.resize(image_descriptor, (M, N),order=0,preserve_range=True)
    ...
    if show_descriptor:
        print('Ahora seleciona 6 puntos de los que quieras ver los descriptores de la imagen original')
    ...
    if do_supervised_clustering:
        ...
        model,sc = generate_supervised_model(image_descriptor_real_size,selected_points,y_class,normalize_data)


        probability_image,segmentation = segment_supervised(image_descriptor,model,normalize_data,sc)
        segmentation_real_size = skimage.transform.resize(segmentation, (M, N), order=0)
